angular.module("carinfo.Ctrl", []).controller("carinfoCtrl", function ($rootScope, $state, serverDeferred, $window, $scope, $ionicSlideBoxDelegate, $ionicHistory, $ionicLoading) {
  ...
  $scope.addtoBasket = function () {
    ...
    if (isEmpty(value)) {
      ...
      $rootScope.alert("Сагсанд нэмэгдлээ", "success");
    } else {
      $rootScope.alert("Сагслагдсан байна", "warning");
    }
  };

  $scope.next = function () {
    $ionicSlideBoxDelegate.$getByHandle("slidebox-trend").next();
  };
  $scope.previous = function () {
    $ionicSlideBoxDelegate.$getByHandle("slidebox-trend").previous();
  };
  $scope.slideChangedTrend = function (index) {
  ...
  $rootScope.getCarDatas = function () {
    $rootScope.carDatas = [];
