...
function getVariableinString(str) {
    ...
    if (!isEmpty(strar))
        strar = strar.toString().replaceAll(',', ";")
    else
        strar = null;
...
function getToMimeType(mime) {
    var mime_map = {
        ...
        'ppt': 'application/vnd.ms-powerpoint', 'ppt': 'application/vnd.ms-office',
...
function searchJsonValue(jsObj, fieldrow, fieldValue) {
    console.log(jsObj);
    return Object.keys(jsObj).filter(function (x) {
...
function searchJsonValueGet(jsObj, fieldrow, fieldValue) {
    return jsObj.filter(
        function (jsObj) { return jsObj[fieldrow].toLowerCase() == fieldValue.toLowerCase() }
    );
}
            ...
            for (index in relVariable) {
                if (relVariable[index].match(/,/g) !== undefined && relVariable[index].match(/,/g) !== null && relVariable[index].match(/,/g).length > 0) {
                    ...
                    var ass = change.map(function (v) {
                        var vs = "[" + v.replace("." + functionName, '').replace('(', '').replace(')', '').trim() + "=]'" + functionName + "';";
                        return vs;
                    });
...
function checkvariable(str, json) {
    ...
    subVariableArray.sort(function (a, b) { if (a && b) return a.split('.').length - b.split('.').length });
...
function stringConvertToFunction(arraName, strfunctions, json, LabelValueName) {
    ...
    var widthEvent = exper.replaceAll("parseint", "parseInt").replaceAll("thisclickbutton('save')", "$scope.SaveBtn()").replaceAll("$rootscope", "$rootScope").replaceAll("close(this)", "$scope.backbutton()").replaceAll("hideprocessdialog()", "").replaceAll('[', arraName).replaceAll('.delete()', functionName).replaceAll('.change()', functionName).replaceAll('.trigger(', 'function(').replaceAll('.run()', 'function()').replaceAll('subkpi.save()', '$rootScope.ItemDtlData.subkpifunction=function()').replaceAll('.click()', functionName).replaceAll(']', '').replaceAll('.val()', '').replaceAll('.show()', 'hideShow="show"').replaceAll("='show'", 'hideShow="show"').replaceAll("='hide'", 'hideShow="hide"').replaceAll('.hide()', 'hideShow="hide"').replaceAll('.disable()', 'disable="disable"').replaceAll("='disable'", 'disable="disable"').replaceAll('.enable()', 'disable="enable"').replaceAll("='enable'", 'disable="enable"').replaceAll(";;", ";").replaceAll('getuserlocation', '$rootScope.getUserLocation');
